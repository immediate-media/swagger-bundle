...
namespace IM\Fabric\Bundle\SwaggerBundle\Tests\Swagger;

use IM\Fabric\Bundle\SwaggerBundle\Swagger\SwaggerDecorator;
use ArrayObject;
use Mockery;
...
class SwaggerDecoratorTest extends TestCase
{
    ...
    public function testNormalizeMergeConfigIntoDocWithParameters(): void
    {
        $swaggerDecorator = new SwaggerDecorator(
            $this->defaultDecorator,
            self::CONFIG_FILE_PATH . 'parameters.yaml',
            new Parser(),
            $this->parameterBag
        );
        ...
        $this->parameterBag->shouldReceive('has')
            ->with('foo')
            ->andReturn(true);
        ...
        $expected = ['basePath' => '/', 'info' => ['title' => 'Test doc FOO TEST']];
